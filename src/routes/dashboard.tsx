...
export const Route = createFileRoute("/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — Snackwize" }] }),
  component: () => <UserGuard><Dashboard /></UserGuard>,
});

function Dashboard() {
  const nav = useNavigate();
  const [user] = useState(() => getUser());
  const handleLogout = () => { logout(); nav({ to: "/" }); };
        ...
        <Tabs defaultValue="orders">
          ...
          <TabsContent value="orders" className="mt-6">
            <div className="grid place-items-center rounded-3xl border border-dashed border-border bg-card py-20 text-center">
              ...
              <Link to="/menu" className="mt-6 inline-flex rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary-dark">Browse Menu</Link>
            </div>
          </TabsContent>
