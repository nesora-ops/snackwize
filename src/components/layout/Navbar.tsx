    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/menu" className="relative hidden rounded-full p-2 text-foreground/70 transition hover:bg-primary-light hover:text-primary-dark sm:inline-flex" aria-label="Cart">
          ...
          {user ? (
            <>
              <Link to="/dashboard" className="hidden text-sm font-medium text-foreground/80 hover:text-primary sm:inline">
            ...
            <>
              <Link to="/login" className="hidden text-sm font-medium text-foreground/80 hover:text-primary sm:inline">Login</Link>
      ...
      {open && (
        <div className="border-t border-border/60 bg-background md:hidden">
          <div className="flex flex-col gap-1 px-4 py-3">
            {NAV.map((n) => (
              <Link key={n.to} to={n.to} onClick={() => setOpen(false)} className="rounded-lg px-3 py-2 text-sm font-medium hover:bg-surface">
                {n.label}
              </Link>
            ))}
            <div className="mt-2 flex gap-2">
              {user ? (
                <>
                  <Link to="/dashboard" onClick={() => setOpen(false)} className="flex-1"><Button variant="outline" className="w-full"><User className="mr-2 h-4 w-4" />Dashboard</Button></Link>
                  <Button onClick={() => { logout(); setOpen(false); }} className="flex-1 bg-primary hover:bg-primary-dark">Logout</Button>
                </>
              ) : (
                <>
                  <Link to="/login" onClick={() => setOpen(false)} className="flex-1"><Button variant="outline" className="w-full">Login</Button></Link>
                  <Link to="/signup" onClick={() => setOpen(false)} className="flex-1"><Button className="w-full bg-primary hover:bg-primary-dark">Sign up</Button></Link>
                </>
              )}
            </div>
          </div>
